#!/bin/bash
# =============================================================================
# deploy.sh — Primer despliegue del LMS Corporativo en red interna (LAN)
#
# Uso:
#   chmod +x deploy.sh && ./deploy.sh
#
# Admin creado automáticamente:
#   Email:    admin@empresa.com
#   Clave:    Demo1234!  (cámbiala en el primer login)
# =============================================================================

set -e

GREEN='\033[0;32m'; BLUE='\033[0;34m'; RED='\033[0;31m'; NC='\033[0m'
info()    { echo -e "${BLUE}[INFO]${NC} $1"; }
success() { echo -e "${GREEN}[OK]${NC}   $1"; }
error()   { echo -e "${RED}[ERROR]${NC} $1"; exit 1; }

echo ""
echo "================================================"
echo "  LMS Corporativo — Despliegue en LAN"
echo "================================================"
echo ""

command -v docker >/dev/null 2>&1 || error "Docker no está instalado."
docker compose version >/dev/null 2>&1 || error "Docker Compose v2 no está disponible."

# ── IP del servidor ──────────────────────────────────────────────────────────
SERVER_IP=$(hostname -I 2>/dev/null | awk '{print $1}')
echo "  IP detectada: ${SERVER_IP}"
read -p "  ¿Es correcta? Si no, escribe la IP real (Enter para confirmar): " INPUT_IP
[ -n "$INPUT_IP" ] && SERVER_IP=$INPUT_IP
[ -z "$SERVER_IP" ] && error "No se pudo determinar la IP del servidor."

# ── Puerto (opcional) ────────────────────────────────────────────────────────
read -p "  Puerto HTTP [80]: " SERVER_PORT
SERVER_PORT=${SERVER_PORT:-80}

if [ "$SERVER_PORT" = "80" ]; then
  BASE_URL="http://${SERVER_IP}"
else
  BASE_URL="http://${SERVER_IP}:${SERVER_PORT}"
fi

# ── Base de datos ─────────────────────────────────────────────────────────────
read -p "  Contraseña para la base de datos: " -s DB_PASSWORD; echo
[ -z "$DB_PASSWORD" ] && error "La contraseña es obligatoria."

# ── Opcionales ────────────────────────────────────────────────────────────────
read -p "  Email SMTP del sistema (Enter para omitir): " SMTP_USER
read -p "  Contraseña SMTP (Enter para omitir): " -s SMTP_PASSWORD; echo
read -p "  API Key Anthropic para IA (Enter para omitir): " ANTHROPIC_KEY

echo ""

# ── SECRET_KEY ────────────────────────────────────────────────────────────────
SECRET_KEY=$(python3 -c "import secrets; print(secrets.token_urlsafe(50))" 2>/dev/null || \
             openssl rand -base64 48 | tr -d '\n')

# ── Generar .env ──────────────────────────────────────────────────────────────
info "Creando .env ..."
cat > .env << EOF
# Generado por deploy.sh — $(date)
COMPOSE_PROJECT_NAME=lms

SECRET_KEY=${SECRET_KEY}
DEBUG=False
ALLOWED_HOSTS=${SERVER_IP},localhost,127.0.0.1

DB_NAME=lms_prod
DB_USER=lms_user
DB_PASSWORD=${DB_PASSWORD}
DATABASE_URL=postgres://lms_user:${DB_PASSWORD}@db:5432/lms_prod

REDIS_URL=redis://redis:6379/0

CORS_ALLOWED_ORIGINS=${BASE_URL}
CSRF_TRUSTED_ORIGINS=${BASE_URL}
FRONTEND_URL=${BASE_URL}
VITE_API_URL=${BASE_URL}

EMAIL_BACKEND=django.core.mail.backends.smtp.EmailBackend
EMAIL_HOST=smtp.office365.com
EMAIL_PORT=587
EMAIL_USE_TLS=True
EMAIL_HOST_USER=${SMTP_USER}
EMAIL_HOST_PASSWORD=${SMTP_PASSWORD}
DEFAULT_FROM_EMAIL=LMS Corporativo <${SMTP_USER}>

ANTHROPIC_API_KEY=${ANTHROPIC_KEY}
LDAP_ENABLED=False
STORAGE_BACKEND=local

SECURE_SSL_REDIRECT=False
SESSION_COOKIE_SECURE=False
CSRF_COOKIE_SECURE=False

LOG_LEVEL=WARNING
LOG_FORMAT=json
EOF

# Actualizar el puerto en nginx si no es 80
if [ "$SERVER_PORT" != "80" ]; then
  sed -i "s/listen 80;/listen ${SERVER_PORT};/" nginx/lan.conf
fi

success ".env creado."

# ── Build y arranque ──────────────────────────────────────────────────────────
info "Construyendo imágenes (puede tardar 3-5 minutos la primera vez) ..."
docker compose build

info "Levantando servicios ..."
docker compose up -d

# ── Esperar que el backend esté listo ─────────────────────────────────────────
info "Esperando que el backend esté listo ..."
ATTEMPTS=0
until docker compose exec -T backend python manage.py check --database default >/dev/null 2>&1; do
  ATTEMPTS=$((ATTEMPTS + 1))
  [ $ATTEMPTS -ge 30 ] && error "El backend no arrancó. Revisa: docker compose logs backend"
  sleep 3
done

echo ""
echo "================================================"
success "¡LISTO!"
echo ""
echo -e "  Accede en: ${GREEN}${BASE_URL}${NC}"
echo ""
echo "  Usuario: admin@empresa.com"
echo "  Clave:   Demo1234!  ← cámbiala en el primer login"
echo ""
echo "  Ver logs: docker compose logs -f"
echo "  Estado:   docker compose ps"
echo "================================================"
