      ...
      <div className="space-y-2">
        {modules.map((module, idx) => (
          <div
            ...
            className={`flex items-center gap-3 p-3 bg-card border rounded-lg cursor-grab
              ${dragOver === idx ? "border-indigo-500/60 bg-indigo-500/10" : "border-border"}`}
          >
            <span className="text-muted-foreground select-none">⠿</span>
            <span className="text-lg">{TIPO_ICONS[module.tipo_contenido]}</span>
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-foreground truncate">{module.titulo}</p>
      ...
      {showForm ? (
        <div className="border border-border rounded-xl p-5 bg-background space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                ...
                className="w-full border border-slate-700 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20 transition-all duration-300"
              />
            </div>
            <div>
          ...
          {form.tipo_contenido === "PDF" && (
            <div>
              <label className="block text-xs font-medium text-muted-foreground mb-1">
                ...
                {editingId && <span className="text-muted-foreground"> (dejar vacío para mantener el actual)</span>}
              </label>
              <input
                type="file"
          ...
          {form.tipo_contenido === "TEXTO" && (
            <div>
              <label className="block text-xs font-medium text-muted-foreground mb-1">Contenido HTML</label>
              <RichTextEditor
          ...
          <div className="flex gap-2 justify-end">
            ...
            <button
              ...
              className="px-4 py-2 bg-indigo-600 text-white text-sm rounded-lg hover:bg-indigo-700 disabled:opacity-50 shadow-lg shadow-indigo-500/20 active:scale-[0.98] transition-all duration-300"
            >
